...
class StandingsCalculator:
    ...
    - API support: Can use current standings from SportMonks API for live predictions
    """
    ...
    def __init__(self, sportmonks_client=None):
        ...
        Args:
            sportmonks_client: Optional SportMonksClient for fetching current standings
        """
        self.standings_cache = {}
        self.sportmonks_client = sportmonks_client
        self.api_standings_cache = {}  # Cache API standings

    def set_api_standings(self, season_id: int, league_id: int, standings_df: pd.DataFrame):
        """
        Set current standings from API (for live predictions).

        This allows using SportMonks API standings instead of recalculating from fixtures.
        Much faster for live predictions where we need current standings.

        Args:
            season_id: Season ID
            league_id: League ID
            standings_df: DataFrame with API standings (from SportMonksClient.get_standings_as_dataframe)
        """
        cache_key = (league_id, season_id)
        self.api_standings_cache[cache_key] = standings_df.copy()
        logger.info(f"Cached API standings for season {season_id}, league {league_id} ({len(standings_df)} teams)")
    ...
    def get_current_standings(
        ...
        """
        Get current league standings (prefers API if available).

        Args:
            season_id: Season ID
            league_id: League ID
            use_api: If True and API standings cached, use those; else return empty

        Returns:
            DataFrame with current standings or empty DataFrame
        """
        cache_key = (league_id, season_id)

        if use_api and cache_key in self.api_standings_cache:
            logger.debug(f"Using cached API standings for season {season_id}, league {league_id}")
            return self.api_standings_cache[cache_key].copy()

        logger.debug(f"No API standings cached for season {season_id}, league {league_id}")
        return pd.DataFrame()
    ...
    def get_standing_features(
        ...
        Tries API standings first (if cached), falls back to calculating from fixtures.
        ...
        """
        # Try API standings first (much faster for live predictions)
        standings = self.get_current_standings(season_id, league_id, use_api=True)

        # Fall back to calculating from fixtures if no API data
        if standings.empty:
            logger.debug(f"No API standings, calculating from fixtures for season {season_id}")
            standings = self.calculate_standings_at_date(
                fixtures_df, season_id, league_id, as_of_date
            )
