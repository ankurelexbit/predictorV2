...
class Pillar2ModernAnalyticsEngine:
    ...
    def _calculate_xg_stats(self, team_id: int, fixtures: pd.DataFrame) -> Dict:
        ...
        for _, match in fixtures.iterrows():
            ...
            shots_inside_box_list.append(team_stats.get('shots_inside_box', 0))
            corners_list.append(team_stats.get('corners', 0))
        ...
        inside_box_5 = shots_inside_box_list[:5]
        corners_5 = corners_list[:5]

        # inside_box_xg_ratio: proportion of shots from inside the box
        total_shots_5 = sum(shots_5)
        total_inside_box_5 = sum(inside_box_5)
        inside_box_ratio = total_inside_box_5 / total_shots_5 if total_shots_5 > 0 else 0.65

        # big_chance_conversion: goals per big chance
        total_goals_5 = sum(goals_5)
        total_big_chances_5 = sum(big_chances_5)
        big_chance_conv = total_goals_5 / total_big_chances_5 if total_big_chances_5 > 0 else 0.3

        # xG from corners: ~0.02 xG per corner (industry standard)
        total_corners_5 = sum(corners_5)
        xg_from_corners = total_corners_5 * 0.02 / len(corners_5) if corners_5 else 0.0
        ...
        return {
            ...
            'inside_box_xg_ratio': float(inside_box_ratio),
            'big_chances_5': float(np.mean(big_chances_5)) if big_chances_5 else 0.0,
            'big_chance_conversion_5': float(big_chance_conv),
            'xg_corners_5': float(xg_from_corners),
    ...
    def _extract_team_stats(self, match: pd.Series, is_home: bool) -> Dict:
        ...
        corners = match.get(f'{prefix}corners', 0) or 0

        # Use raw big_chances from data (type 580) when available
        big_chances_raw = match.get(f'{prefix}big_chances', 0) or 0
        if big_chances_raw > 0:
            big_chances = big_chances_raw
        elif shots_total > 0:
            # Fallback: estimate from shot accuracy
    ...
    def _calculate_defensive_stats(self, team_id: int, fixtures: pd.DataFrame) -> Dict:
        ...
        for _, match in fixtures[:5].iterrows():
            ...
            possession = match.get(f'{prefix}possession', 0) or 0
            opp_passes = match.get(f'{opp_prefix}passes', 0) or 0
            fouls = match.get(f'{prefix}fouls', 0) or 0

            tackles_list.append(tackles)
            interceptions_list.append(interceptions)
            possession_list.append(possession)
            opp_passes_list.append(opp_passes)
        ...
        avg_opp_passes = np.mean(opp_passes_list) if opp_passes_list else 0.0
        avg_fouls = np.mean(fouls_list) if fouls_list else 0.0

        # PPDA: opponent passes per defensive action (lower = more pressing)
        def_actions = avg_tackles + avg_interceptions
        ppda = avg_opp_passes / def_actions if def_actions > 0 else 10.5

        # Tackle success rate: approximate as 1 - (fouls / (tackles + fouls))
        tackle_total = avg_tackles + avg_fouls
        tackle_success = 1 - (avg_fouls / tackle_total) if tackle_total > 0 else 0.75

        return {
            'ppda': float(ppda),
            'tackles': float(avg_tackles),
            'interceptions': float(avg_interceptions),
            'tackle_success': float(tackle_success),
            'defensive_actions': float(def_actions),
