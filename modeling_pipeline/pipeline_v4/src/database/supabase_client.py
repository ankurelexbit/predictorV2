...
Handles storing and retrieving predictions from Supabase PostgreSQL database.
"""
...
class SupabaseClient:
    """Client for storing predictions in Supabase."""
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS predictions (
                    ...
                    -- Features used for prediction (162 features stored as JSONB)
                    features JSONB,
    ...
    def _clean_nan_values(self, obj):
        ...
        PostgreSQL JSON type doesn't accept NaN values.
        """
        import math
    ...
    def update_actual_result(self, fixture_id: int, home_score: int, away_score: int,
                            model_version: str = 'v4_conservative'):
        """
        Update prediction with actual match result.
        Updates ALL model versions for the fixture (actual result is the same for all).

        Args:
            fixture_id: Fixture ID
            home_score: Actual home team score
            away_score: Actual away team score
            model_version: Model version (kept for backwards compatibility, but updates all versions)
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            ...
            # Get ALL predictions for this fixture (all model versions)
            cursor.execute("""
            ...
            # Update each model version's prediction
            updated_count = 0
            for row in results:
                ...
                if should_bet and bet_outcome:
                    # Map bet outcome to result code and get the actual market odds
                    bet_code = {'Home Win': 'H', 'Draw': 'D', 'Away Win': 'A'}.get(bet_outcome)

                    # Use best market odds for the bet outcome
                    ...
                    if market_odds and bet_won:
                        bet_profit = (market_odds - 1) * 1.0  # Assume $1 stake
                    elif market_odds:
                        bet_profit = -1.0
                    else:
                        bet_profit = None

                # Update record for this model version
                cursor.execute("""
                    UPDATE predictions
            ...
            logger.info(f"✅ Updated actual result for fixture {fixture_id} ({updated_count} model version(s))")

    def update_actual_results_batch(self, results: Dict[int, Dict]) -> int:
        """
        Update multiple fixtures with actual results in one batch operation.
        Much faster than updating one at a time.
        ...
        with self.get_connection() as conn:
            ...
            # Get all predictions for all fixtures in one query
            ...
            if not predictions:
                logger.warning(f"No predictions found for {len(fixture_ids)} fixtures")
                return 0
            ...
            # Single bulk update for all predictions
            if update_values:
                from psycopg2.extras import execute_values
                execute_values(cursor, """
                ...
                logger.info(f"✅ Batch updated {len(update_values)} predictions for {len(results)} fixtures")
                return len(update_values)

            return 0

    def get_pending_results(self, model_version: str = 'v4_conservative') -> List[Dict]:
        """
        ...
        Returns:
            List of predictions without results
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
    ...
    def get_betting_performance(self, days: int = 30, model_version: str = 'v4_conservative') -> Dict:
        """
    ...
    def get_detailed_pnl(self, start_date: str = None, end_date: str = None,
                         model_version: str = 'v4_conservative') -> Dict:
        """
        Get detailed PnL breakdown with market odds.
        ...
        Returns:
            Dictionary with detailed PnL metrics
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
