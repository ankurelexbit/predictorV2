"""
Comprehensive Model Optimization with Class Weights.

This script:
1. Trains models with class weights (1.8x Draw, 1.3x Away, 1.0x Home)
2. Tests XGBoost, LightGBM, CatBoost
...
# Class weights: Away=1.3, Draw=1.8, Home=1.0
CLASS_WEIGHTS = {0: 1.3, 1: 1.8, 2: 1.0}
...
class ModelComparison:
    """Comprehensive model comparison and optimization with class weights."""

    def __init__(self, data_path: str, n_trials: int = 50, class_weights: dict = None):
    ...
    def _load_data(self):
        ...
        logger.info(f"  Away (0): {self.class_weights[0]:.1f}x")
        logger.info(f"  Draw (1): {self.class_weights[1]:.1f}x")
        logger.info(f"  Home (2): {self.class_weights[2]:.1f}x")
    ...
    def _prepare_data(self):
        ...
        self.train_weights = get_sample_weights(self.y_train, self.class_weights)
        self.val_weights = get_sample_weights(self.y_val, self.class_weights)
        self.test_weights = get_sample_weights(self.y_test, self.class_weights)
    ...
    def train_baseline_xgboost(self):
        ...
        logger.info(f"\n{'=' * 80}")
        logger.info("1. BASELINE XGBOOST (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
        ...
        model.fit(self.X_train, self.y_train, sample_weight=self.train_weights)
        ...
        logger.info(f"Test  - Draw Acc: {test_metrics['draw_accuracy']:.4f} (Target: >0.20)")
    ...
    def train_baseline_lightgbm(self):
        """Train baseline LightGBM model with class weights."""
        logger.info(f"\n{'=' * 80}")
        logger.info("2. BASELINE LIGHTGBM (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
    ...
    def train_baseline_catboost(self):
        """Train baseline CatBoost model with class weights."""
        logger.info(f"\n{'=' * 80}")
        logger.info("3. BASELINE CATBOOST (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
    ...
    def optimize_xgboost(self):
        """Hyperparameter optimization for XGBoost with class weights."""
        logger.info(f"\n{'=' * 80}")
        logger.info("4. XGBOOST HYPERPARAMETER OPTIMIZATION (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
    ...
    def optimize_lightgbm(self):
        """Hyperparameter optimization for LightGBM with class weights."""
        logger.info(f"\n{'=' * 80}")
        logger.info("5. LIGHTGBM HYPERPARAMETER OPTIMIZATION (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
        ...
        # Train final model with best params
        best_params = study.best_params.copy()
        best_params.update({'objective': 'multiclass', 'num_class': 3, 'class_weight': self.class_weights,
    ...
    def optimize_catboost(self):
        """Hyperparameter optimization for CatBoost with class weights."""
        logger.info(f"\n{'=' * 80}")
        logger.info("6. CATBOOST HYPERPARAMETER OPTIMIZATION (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
        ...
        # Train final model with best params
        best_params = study.best_params.copy()
        best_params.update({'loss_function': 'MultiClass', 'class_weights': list(self.class_weights.values()),
    ...
    def train_stacking_ensemble(self):
        ...
        logger.info(f"\n{'=' * 80}")
        logger.info("7. STACKING ENSEMBLE (WITH CLASS WEIGHTS)")
        logger.info(f"{'=' * 80}\n")
        ...
        meta_learner = LogisticRegression(max_iter=1000, random_state=42, class_weight=self.class_weights)
    ...
    def generate_report(self, output_path: str = None):
        ...
        print("=" * 140)
        print(f"{'Model':<30} {'Test Acc':<12} {'Test LogLoss':<15} {'Draw Acc':<12} {'Val Acc':<12} {'Val LogLoss':<15}")
        print("-" * 140)

        for _, row in df_comparison.iterrows():
            print(f"{row['Model']:<30} {row['Test_Accuracy']:>10.4f}  {row['Test_LogLoss']:>13.4f}  "
                  f"{row['Test_Draw_Acc']:>10.4f}  {row['Val_Accuracy']:>10.4f}  {row['Val_LogLoss']:>13.4f}")
        ...
        draw_improvement = (best['Test_Draw_Acc'] - baseline['Test_Draw_Acc']) * 100
        ...
        logger.info(f"   Draw Accuracy: {draw_improvement:+.2f} percentage points")
        ...
        if output_path:
            report_data = {
                ...
                'summary': {
                    ...
                    'test_draw_accuracy': float(best_model['Test_Draw_Acc']),
                    'improvement_vs_baseline': {
                        'accuracy_pp': float(acc_improvement),
                        'logloss': float(logloss_improvement),
                        'draw_accuracy_pp': float(draw_improvement)
                    }
                }
            }
...
def main():
    parser = argparse.ArgumentParser(description='Comprehensive Model Optimization with Class Weights')
    parser.add_argument('--data', type=str, default='data/training_data.csv',
                       help='Path to training data')
    parser.add_argument('--n-trials', type=int, default=50,
                       help='Number of Optuna trials for hyperparameter tuning')
    parser.add_argument('--output-report', type=str, default='results/model_comparison_weighted.json',
                       help='Path to save comparison report')
    parser.add_argument('--output-model', type=str, default='models/v4_optimized_weighted_model.joblib',
                       help='Path to save best model')
    parser.add_argument('--skip-tuning', action='store_true',
                       help='Skip hyperparameter tuning (faster)')
    parser.add_argument('--away-weight', type=float, default=1.3,
                       help='Class weight for away wins')
    parser.add_argument('--draw-weight', type=float, default=1.8,
                       help='Class weight for draws')
    parser.add_argument('--home-weight', type=float, default=1.0,
                       help='Class weight for home wins')
    ...
    # Set class weights
    class_weights = {0: args.away_weight, 1: args.draw_weight, 2: args.home_weight}

    # Run comparison
    comparison = ModelComparison(args.data, n_trials=args.n_trials, class_weights=class_weights)
